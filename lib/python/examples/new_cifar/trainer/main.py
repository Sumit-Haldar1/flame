...
class HorizontalSplitTrainer(Trainer):
    ...
    #     ])
    #     dataset = datasets.KMNIST('./data', train=True, download=True, transform=transform)
    ...
    def load_data(self):
        ...
        try:
            ...
            indices = torch.arange(self.rank*5000, min((self.rank+1)*5000, len(dataset)))
            subset = data_utils.Subset(dataset, indices)
            self.train_loader = data_utils.DataLoader(subset, batch_size=self.batch_size, shuffle=True)
    ...
    def train(self) -> None:
        """Train a model."""
        self.optimizer = optim.Adadelta(self.model.parameters(), lr=0.05)
